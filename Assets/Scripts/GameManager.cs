...
public enum EnemyModifier { None, Armored, Enraged, Regen, Cursed, Spectral, Leech, Volatile, Fortified, Giant, Blessed, Frenzied, Mirrored, Phantom, Colossus, Stalwart, Bloodthirsty, Warded, Explosive, Tenacious, Phasic, Swift, Desperate }
public enum BossAbility    { None, Shield, Berserk, Drain, Regen, Thorns, Haste, Wrath, Miasma, LeechStrike, Venom, Hex, Bulwark, CursedAura, BloodTribute, Vampiric, Encase, Overpower, Sunder, Petrify, Sacrifice }
...
public class GameManager : MonoBehaviour
{
    ...
    public int   EffectiveComboMaxStacks  => ComboMaxStacks + SSRuneSealLevel * SSRuneSealStackBonus;
    public float EffectiveComboWindowSecs => ComboWindowSecs + PBattleRhythmLevel * PBattleRhythmBonus;
    ...
    public double WaveBloodPreview
    {
        get
        {
            ...
            if (CurrentEnemyModifier == EnemyModifier.Desperate)      r = Math.Floor(r * EnemyDesperateRewardMult);
    ...
    public float TotalAttack         => (TankCount     * (SoldierAttack   + EquipAttackBonus + VeteranAttackBonus + AchievementAttackBonus + WarDrumAttackBonus + IronMarrowAttackBonus)
                                       ...
                                       * (1f + PrestigeMilestoneDmgBonus) * MoraleBonusMult * WarMachineMult * (1f + PWarFervorLevel * PWarFervorBonus);
    ...
    public float  EquipAttackBonus    => WeaponLevel   * (3f + PForgeMasterLevel * PForgeMasterBonus);
    ...
    public int PBattleRhythmLevel        { get; private set; }
    ...
    public int PForgeMasterLevel         { get; private set; }
    ...
    public int PBloodFrenzyLevel             { get; private set; }
    ...
    public int    SSRuneSealLevel       { get; private set; }
    public const int SSRuneSealStackBonus = 2; // +2 max combo stacks per level
    public int    SSWarCrestLevel       { get; private set; }
    ...
    public int    SSDeathWardLevel      { get; private set; }
    public const float SSDeathWardReviveChance = 0.05f; // 5% revive chance per level on soldier death
    ...
    public int    SSShadowSurgeLevel    { get; private set; }
    ...
    public int    SSBloodSigilLevel     { get; private set; }
    public const float SSBloodSigilDmgMult = 1.0f; // HealSelf also deals 100pct of HP healed as damage per level
    public int    SSEchoBlastLevel      { get; private set; }  // free Blood Storm casts when Surge expires
    public int    SSSoulRendLevel       { get; private set; }
    public const float SSSoulRendAtkReduction = 0.20f; // 20pct enemy atk reduction per level
    public const float SSSoulRendDuration     = 5f;    // seconds debuff lasts after Blood Storm
    public bool SoulRendActive => _soulRendTimer > 0f;
    public int    SSLifeTapLevel        { get; private set; }
    public const float SSLifeTapHealPct = 0.05f; // frontline heals 5pct of damage dealt per level
    public int    SSSoulTaxLevel        { get; private set; }
    ...
    public int    SSStormFrontLevel     { get; private set; }
    public const float SSStormFrontDuration = 3f; // seconds of silence per level after Blood Storm
    ...
    public double EffectiveBankInterestRate => BankInterestRatePerHour + BankInterestLevel * BankInterestRatePerLevel + PCompoundInterestLevel * PCompoundInterestBonus;
    public double BankMaxDeposit => BankMaxDepositBase * Math.Pow(10.0, PrestigeCount * 0.5) * (1.0 + PVaultExpansionLevel * PVaultExpansionBonus);

    // --- Wave Streak ---
    public int   WaveStreak          { get; private set; }
    public int   BestWave            { get; private set; }
    public int   BestStreak          { get; private set; }
    public float StreakMultiplierCap => MaxStreakMultiplier + BannerLevel * BannerStreakCapBonus + SSWarCrestLevel * SSWarCrestBonus;
    ...
    float _stormFrontTimer;
    public bool StormFrontActive => _stormFrontTimer > 0f;
    float _bossPetrifyTimer;
    float _bossPetrifyActiveTimer;
    float _bossSacrificeTimer;
    ...
    public const float  BossVampiricLifestealPct = 0.30f; // boss heals 30pct of damage dealt to soldiers
    public const double BossVampiricRewardMult   = 2.00;  // 2x kill reward
    ...
    public const float  BossSacrificeInterval    = 25f;   // seconds between sacrifice bursts
    public const float  BossSacrificeSelfPct     = 0.10f; // pct of boss max HP sacrificed per burst
    public const float  BossSacrificeDmgMult     = 2.00f; // damage dealt = sacrificed HP × this
    public const double BossSacrificeRewardMult  = 2.50;  // 2.5x kill reward
    ...
    public const float BossLeechStrikeHealPct  = 0.20f; // heals 20% of damage dealt
    ...
    public string BossAbilityDisplay => CurrentBossAbility switch
    {
        ...
        BossAbility.Sacrifice    => "💀 Sacrifice",
        _                   => "",
    };
    ...
    public float  WarCryDurationEffective    => WarCryDuration    + WarCryUpgradeLevel    * 5f + SSWarHornLevel * SSWarHornDuration + PBloodFrenzyLevel * PBloodFrenzyBonusSecs;
    ...
    public string EnemyModifierDisplay => CurrentEnemyModifier switch
    {
        ...
        EnemyModifier.Desperate     => "😤 Desperate",
        _                     => "",
    };
    ...
    public const float  EnemyPhantomDmgNegate    = 0.20f; // 20% of soldier damage is negated
    ...
    public const float  EnemyDesperatePeakMult        = 2.00f; // attack multiplier at 0 HP (scales from 1x at full HP)
    public const double EnemyDesperateRewardMult      = 1.80;  // 1.8x kill reward
    ...
    float _phasicTimer;
    float _phasicActiveTimer;
    public bool PhasicActive { get; private set; }
    ...
    void Update()
    {
        ...
        if (SurgeActive)
        {
            ...
            {
                SurgeActive = false; SurgeTimeRemaining = 0f;
                if (SSEchoBlastLevel > 0 && BloodStormUnlocked && EnemyHP > 0 && SoldierCount > 0)
                    for (int i = 0; i < SSEchoBlastLevel; i++) FireEchoBlast();
            }
            changed = true;
        }
    ...
    bool RunCombat(float dt)
    {
        ...
        if (_soulRendTimer    > 0f) { _soulRendTimer    -= dt; if (_soulRendTimer    < 0f) _soulRendTimer    = 0f; }
        if (_stormFrontTimer  > 0f) { _stormFrontTimer  -= dt; if (_stormFrontTimer  < 0f) _stormFrontTimer  = 0f; }
        if (CurrentEnemyModifier == EnemyModifier.Phasic && EnemyHP > 0)
        {
            if (PhasicActive) { _phasicActiveTimer -= dt; if (_phasicActiveTimer <= 0f) { PhasicActive = false; _phasicTimer = EnemyPhasicInterval; } }
            else { _phasicTimer -= dt; if (_phasicTimer <= 0f) { PhasicActive = true; _phasicActiveTimer = EnemyPhasicDuration; } }
        }
        ...
        else
        {
            ...
            if (SSLifeTapLevel > 0 && SoldierCount > 0 && dmgThisTick > 0)
                SoldierHP = Mathf.Min(SoldierHP + dmgThisTick * SSLifeTapLevel * SSLifeTapHealPct, FrontlineMaxHP);
        }
        if (SurgeActive && SSShadowSurgeLevel > 0 && EnemyHP > 0)
            EnemyHP = Mathf.Max(0f, EnemyHP - SSShadowSurgeLevel * SSShadowSurgeDmgPerSec * dt);
        ...
        if (EnemyHP <= 0f)
        {
            ...
            if (wasBoss)
            {
                ...
                if (CurrentBossAbility == BossAbility.Sacrifice)     reward = Math.Floor(reward * BossSacrificeRewardMult);
        ...
        if (WaveStreak >= 25) TryUnlock(AchievementFlags.StreakLegend);
            ...
            if (PSoulTideLevel > 0 && SoldierCount > 0)
                SoldierHP = Mathf.Min(SoldierHP + EnemyMaxHP * PSoulTideLevel * PSoulTideHealPct, FrontlineMaxHP);
            AddBlood(reward);
            ...
            if (SSKillSurgeLevel > 0 && SurgeActive)
                SurgeTimeRemaining += SSKillSurgeLevel * SSKillSurgeSecs;
        ...
        if (CurrentEnemyModifier == EnemyModifier.Bloodthirsty && EnemyMaxHP > 0)
            incomingAtk *= Mathf.Lerp(1f, EnemyBloodthirstyAtkMult, EnemyHP / EnemyMaxHP);
        if (CurrentEnemyModifier == EnemyModifier.Desperate && EnemyMaxHP > 0)
            incomingAtk *= Mathf.Lerp(EnemyDesperatePeakMult, 1f, EnemyHP / EnemyMaxHP);
        ...
        if (SSSoulRendLevel > 0 && SoulRendActive) incomingAtk *= (1f - SSSoulRendLevel * SSSoulRendAtkReduction);
        if (SSStormFrontLevel > 0 && StormFrontActive) incomingAtk = 0f;
        ...
        if (isSpecialFoe && CurrentBossAbility == BossAbility.Encase && EnemyHP > 0)
        {
            if (BossEncaseActive)
            {
                _bossEncaseActiveTimer -= dt;
                if (_bossEncaseActiveTimer <= 0f) BossEncaseActive = false;
            }
            else
            {
                _bossEncaseTimer -= dt;
                if (_bossEncaseTimer <= 0f)
                {
                    BossEncaseActive       = true;
        ...
        if (isSpecialFoe && CurrentBossAbility == BossAbility.Sunder && EnemyHP > 0 && SoldierCount > 0)
        {
            _bossSunderTimer -= dt;
            if (_bossSunderTimer <= 0f)
            {
        ...
        if (isSpecialFoe && CurrentBossAbility == BossAbility.Sacrifice && EnemyHP > 0 && SoldierCount > 0)
        {
            _bossSacrificeTimer -= dt;
            if (_bossSacrificeTimer <= 0f && EnemyHP > EnemyMaxHP * BossSacrificeSelfPct)
            {
                _bossSacrificeTimer = BossSacrificeInterval;
                float sacrificed = EnemyMaxHP * BossSacrificeSelfPct;
                EnemyHP -= sacrificed;
                float burstDmg = sacrificed * BossSacrificeDmgMult;
                if (BloodShieldHP > 0f) { float abs = Mathf.Min(BloodShieldHP, burstDmg); BloodShieldHP -= abs; burstDmg -= abs; }
                SoldierHP = Mathf.Max(0f, SoldierHP - burstDmg);
            }
        }
        ...
        if (isSpecialFoe && CurrentBossAbility == BossAbility.LeechStrike && dmg > 0 && EnemyHP > 0)
            EnemyHP = Mathf.Min(EnemyHP + dmg * BossLeechStrikeHealPct, EnemyMaxHP);
        if (isSpecialFoe && CurrentBossAbility == BossAbility.Vampiric && dmg > 0 && EnemyHP > 0)
            EnemyHP = Mathf.Min(EnemyHP + dmg * BossVampiricLifestealPct, EnemyMaxHP);
        ...
        if (SoldierHP <= 0f)
        {
            ...
            else if (SSDeathWardLevel > 0 && UnityEngine.Random.value < SSDeathWardLevel * SSDeathWardReviveChance)
            {
                SoldierHP = FrontlineMaxHP;
            }
            else
            {
    ...
    void SpawnEnemy(int wave)
    {
        ...
        PhasicActive         = false;
        _phasicTimer         = EnemyPhasicInterval;
        ...
        if (isBoss)
        {
            ...
            CurrentBossAbility   = (BossAbility)UnityEngine.Random.Range(0, 21);
        ...
        else
        {
            ...
            {
                CurrentEnemyModifier = (EnemyModifier)UnityEngine.Random.Range(1, 23);
                ...
                else if (CurrentEnemyModifier == EnemyModifier.Colossus)
                {
    ...
    public void FarmBlood()
    {
        ...
        if (ComboStacks < EffectiveComboMaxStacks) ComboStacks++;
        _comboTimer = EffectiveComboWindowSecs;
    ...
    public bool UseBloodStorm()
    {
        ...
        if (SSSoulRendLevel > 0) _soulRendTimer = SSSoulRendDuration;
        if (SSStormFrontLevel > 0) _stormFrontTimer = SSStormFrontLevel * SSStormFrontDuration;
        _dailySpellCount++;
        TotalSpellsCast++;
    ...
    public bool UseBloodShield()
    {
        ...
        BloodShieldHP = BloodShieldAmount * BoneWardShieldMult;
        OnStateChanged?.Invoke();
        return true;
    }
    ...
    public bool UseHealSelf()
    {
        if (!HealSelfUnlocked || Blood < HealSelfCost || SoldierCount == 0 || SoldierHP >= FrontlineMaxHP)
            return false;
        Blood -= HealSelfCost;
        float healAmt = HealSelfAmountEffective;
        SoldierHP = Mathf.Min(SoldierHP + healAmt, FrontlineMaxHP);
        if (SSBloodSigilLevel > 0 && EnemyHP > 0)
            EnemyHP = Mathf.Max(float.Epsilon, EnemyHP - healAmt * SSBloodSigilLevel * SSBloodSigilDmgMult);
        OnStateChanged?.Invoke();
        return true;
    }
    ...
    public bool UseSurge()
    {
        ...
        SurgeActive = true;
        SurgeTimeRemaining = SurgeDurationEffective;
        if (SSSurgeShieldLevel > 0)
            BloodShieldHP = Mathf.Min(BloodShieldHP + FrontlineMaxHP * SSSurgeShieldLevel * SSSurgeShieldPct, FrontlineMaxHP);
        _dailySpellCount++;
        TotalSpellsCast++;
        if (TotalSpellsCast >= 50)   TryUnlock(AchievementFlags.SpellCaster);
        if (TotalSpellsCast >= 500)  TryUnlock(AchievementFlags.GrandWizard);
    ...
    public bool BuyPVoidPact()
    {
        if (PrestigePoints < PrestigeShopCost || PVoidPactLevel >= 3) return false;
        PrestigePoints -= PrestigeShopCost;
        PVoidPactLevel++;
        OnStateChanged?.Invoke();
        return true;
    ...
    public bool BuyPWarFervor()
    {
        if (PrestigePoints < PrestigeShopCost || PWarFervorLevel >= 3) return false;
        PrestigePoints -= PrestigeShopCost;
        PWarFervorLevel++;
        OnStateChanged?.Invoke();
        return true;
    ...
    public bool BuyPBattleRhythm()
    {
        if (PrestigePoints < PrestigeShopCost || PBattleRhythmLevel >= 3) return false;
        PrestigePoints -= PrestigeShopCost;
        PBattleRhythmLevel++;
        OnStateChanged?.Invoke();
        return true;
    ...
    public bool BuyPSoulTide()
    {
        if (PrestigePoints < PrestigeShopCost || PSoulTideLevel >= 3) return false;
        PrestigePoints -= PrestigeShopCost;
        PSoulTideLevel++;
        OnStateChanged?.Invoke();
        return true;
    ...
    public bool BuyPForgeMaster()
    {
        if (PrestigePoints < PrestigeShopCost || PForgeMasterLevel >= 3) return false;
        PrestigePoints -= PrestigeShopCost;
        PForgeMasterLevel++;
        OnStateChanged?.Invoke();
        return true;
    ...
    public bool BuyPVaultExpansion()
    {
        if (PrestigePoints < PrestigeShopCost || PVaultExpansionLevel >= 3) return false;
        PrestigePoints -= PrestigeShopCost;
        PVaultExpansionLevel++;
        OnStateChanged?.Invoke();
        return true;
    }
    ...
    public bool BuyPBloodFrenzy()
    {
        if (PrestigePoints < PrestigeShopCost || PBloodFrenzyLevel >= 3) return false;
        PrestigePoints -= PrestigeShopCost;
        PBloodFrenzyLevel++;
        OnStateChanged?.Invoke();
        return true;
    ...
    public bool BuyPCompoundInterest()
    {
        if (PrestigePoints < PrestigeShopCost || PCompoundInterestLevel >= 3) return false;
        PrestigePoints -= PrestigeShopCost;
        PCompoundInterestLevel++;
        OnStateChanged?.Invoke();
        return true;
    }
    ...
    public bool BuySSRuneSeal()
    {
        if (SoulShards < SSUpgradeCost || SSRuneSealLevel >= SSMaxLevel) return false;
        SoulShards -= SSUpgradeCost;
        SSRuneSealLevel++;
        OnStateChanged?.Invoke();
        return true;
    ...
    public bool BuySSWarCrest()
    {
        if (SoulShards < SSUpgradeCost || SSWarCrestLevel >= SSMaxLevel) return false;
        SoulShards -= SSUpgradeCost;
        SSWarCrestLevel++;
        OnStateChanged?.Invoke();
        return true;
    ...
    public bool BuySSVitalSurge()
    {
        if (SoulShards < SSUpgradeCost || SSVitalSurgeLevel >= SSMaxLevel) return false;
        SoulShards -= SSUpgradeCost;
        SSVitalSurgeLevel++;
        OnStateChanged?.Invoke();
        return true;
    ...
    public bool BuySSDeathWard()
    {
        if (SoulShards < SSUpgradeCost || SSDeathWardLevel >= SSMaxLevel) return false;
        SoulShards -= SSUpgradeCost;
        SSDeathWardLevel++;
        OnStateChanged?.Invoke();
        return true;
    ...
    public bool BuySSBoneWard()
    {
        if (SoulShards < SSTier2Cost || SSBoneWardLevel >= SSTier2MaxLevel) return false;
        SoulShards -= SSTier2Cost;
        SSBoneWardLevel++;
        OnStateChanged?.Invoke();
        return true;
    ...
    public bool BuySSShadowSurge()
    {
        if (SoulShards < SSTier2Cost || SSShadowSurgeLevel >= SSTier2MaxLevel) return false;
        SoulShards -= SSTier2Cost;
        SSShadowSurgeLevel++;
        OnStateChanged?.Invoke();
        return true;
    ...
    public bool BuySSKillSurge()
    {
        if (SoulShards < SSUpgradeCost || SSKillSurgeLevel >= SSMaxLevel) return false;
        SoulShards -= SSUpgradeCost;
        SSKillSurgeLevel++;
        OnStateChanged?.Invoke();
        return true;
    ...
    public bool BuySSBloodSigil()
    {
        if (SoulShards < SSUpgradeCost || SSBloodSigilLevel >= SSMaxLevel) return false;
        SoulShards -= SSUpgradeCost;
        SSBloodSigilLevel++;
        OnStateChanged?.Invoke();
        return true;
    ...
    public bool BuySSEchoBlast()
    {
        if (SoulShards < SSTier2Cost || SSEchoBlastLevel >= SSTier2MaxLevel) return false;
        SoulShards -= SSTier2Cost;
        SSEchoBlastLevel++;
        OnStateChanged?.Invoke();
        return true;
    ...
    public bool BuySSSoulRend()
    {
        if (SoulShards < SSTier2Cost || SSSoulRendLevel >= SSTier2MaxLevel) return false;
        SoulShards -= SSTier2Cost;
        SSSoulRendLevel++;
        OnStateChanged?.Invoke();
        return true;
    ...
    public bool BuySSLifeTap()
    {
        if (SoulShards < SSUpgradeCost || SSLifeTapLevel >= SSMaxLevel) return false;
        SoulShards -= SSUpgradeCost;
        SSLifeTapLevel++;
        OnStateChanged?.Invoke();
        return true;
    ...
    public bool BuySSSoulTax()
    {
        if (SoulShards < SSUpgradeCost || SSSoulTaxLevel >= SSMaxLevel) return false;
        SoulShards -= SSUpgradeCost;
        SSSoulTaxLevel++;
        OnStateChanged?.Invoke();
        return true;
    ...
    public bool BuySSSurgeShield()
    {
        if (SoulShards < SSTier2Cost || SSSurgeShieldLevel >= SSTier2MaxLevel) return false;
        SoulShards -= SSTier2Cost;
        SSSurgeShieldLevel++;
        OnStateChanged?.Invoke();
        return true;
    ...
    public bool BuySSStormFront()
    {
        if (SoulShards < SSTier2Cost || SSStormFrontLevel >= SSTier2MaxLevel) return false;
        SoulShards -= SSTier2Cost;
        SSStormFrontLevel++;
        OnStateChanged?.Invoke();
        return true;
    ...
    void Save()
    {
        ...
        PlayerPrefs.SetInt   ("SSDeathWardLevel",      SSDeathWardLevel);
        ...
        PlayerPrefs.SetInt   ("SSBloodSigilLevel",     SSBloodSigilLevel);
        PlayerPrefs.SetInt   ("SSEchoBlastLevel",      SSEchoBlastLevel);
        PlayerPrefs.SetInt   ("SSSoulRendLevel",       SSSoulRendLevel);
        ...
        PlayerPrefs.SetInt   ("SSStormFrontLevel",     SSStormFrontLevel);
    ...
    void Load()
    {
        ...
        SSBloodSigilLevel   = PlayerPrefs.GetInt("SSBloodSigilLevel",   0);
        SSEchoBlastLevel    = PlayerPrefs.GetInt("SSEchoBlastLevel",    0);
        SSSoulRendLevel     = PlayerPrefs.GetInt("SSSoulRendLevel",     0);
        ...
        SSStormFrontLevel   = PlayerPrefs.GetInt("SSStormFrontLevel",   0);
        ...
        SSDeathWardLevel    = PlayerPrefs.GetInt("SSDeathWardLevel",    0);
